...
3. В открывшееся окно перетащите файл [openApiDocs](../openApiDocs.json) из корневой папки проекта
4. Дождитесь загрузки и нажмите ```Import```

Во вкладке Collections в Postman появится наша коллекция  ```Gazon API```

> Важно! openApiDocs.json генерируется и обновляется при каждом запуске приложения