...
public class RoleView extends VerticalLayout {
    ...
    public RoleView(RoleService roleService, RoleMapper roleMapper) {
        this.roleService = roleService;
        this.roleMapper = roleMapper;
        this.dataSource = roleService.findAll().stream().map(roleMapper::toDto).collect(Collectors.toList());
        ...
        return grid.addComponentColumn(roleDto -> {
            ...
            deleteButton.addClickListener(e -> {
                ...
                if (grid.getDataProvider().isInMemory() && grid.getDataProvider().getClass() == ListDataProvider.class) {
                    ...
                    Role roleToDelete = roleService.findById(roleDto.getId()).orElseThrow(() -> new RuntimeException("Role not found"));
    ...
    private void addEditorListeners() {
        editor.addSaveListener(e -> {
            Role roleDto = roleMapper.toEntity(e.getItem());
    ...
    private Tab createCreateTab(FormLayout formLayout) {
        ...
        createButton.addClickListener(event -> {
            Role role = new Role();
            role.setName(roleNameField.getValue());
            Role savedRole = roleService.save(role);
            dataSource.add(roleMapper.toDto(savedRole));
            roleNameField.clear();
            grid.getDataProvider().refreshAll();
        });
        return createTab;
